...
{% hook 'component:product.grid.end' %}`;
  ...
  writeOpt('image', `<input name="options[{{ option.id }}]" class="option-image" {% if option.value %}data-default="{{ option.value }}"{% endif %} {{ option.attributes|raw }} type="file" />`);
  writeOpt('multiple-options', `{% if option and option.details %}\n<div class="option option-multiple">{% for key,detail in option.details %}<label><input {{ option.disabled?'disabled':'' }} {{ detail.is_selected?'checked':'' }} name="options[{{ option.id }}]" data-option="{{ option.id }}" value="{{ detail.id }}" type="checkbox" {{ option.attributes|raw }} /> <span>{{ detail.full_name | default(detail.name) }}</span></label>{% endfor %}</div>{% endif %}`);
  writeOpt('number', `<input placeholder="{{ option.placeholder }}" name="options[{{ option.id }}]" value="{{ option.value }}" type="text" {{ option.attributes|raw }} />`);
