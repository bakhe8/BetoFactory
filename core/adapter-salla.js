...
{% hook 'component:product.grid.end' %}`;
  ...
  writeOpt('color', `{% if option and option.details %}\n<div class="option option-color" data-option-id="{{ option.id }}">\n  {% for detail in option.details %}\n  <label class="color-item">\n    <input type="radio" name="options[{{ option.id }}]" value="{{ detail.id }}" {{ detail.is_selected?'checked':'' }} {{ detail.is_out?'disabled':'' }} {{ option.condition_attributes|raw }} />\n    <span class="swatch" style="background: {{ detail.value | default('#ccc') }}"></span>\n    <span class="name">{{ detail.name }}</span>\n    {% if detail.is_out %}<small>Out of stock</small>{% endif %}\n  </label>\n  {% endfor %}\n</div>\n{% endif %}`);
  writeOpt('date', `<input class="form-input date-element{{ option.required?' required':'' }}" placeholder="{{ option.placeholder }}" name="options[{{ option.id }}]" {{ option.attributes|raw }} {{ option.condition_attributes|raw }} readonly="readonly" value="{{ option.value }}" type="text" />`);
  writeOpt('datetime', `<input class="form-input date-time-element{{ option.required?' required':'' }}" placeholder="{{ option.placeholder }}" name="options[{{ option.id }}]" {{ option.attributes|raw }} {{ option.condition_attributes|raw }} readonly="readonly" value="{{ option.value }}" type="text" />`);
  writeOpt('donation', `{% if product and product.can_donate %}\n<div class="option option-donation"><label>Donation Amount</label><input type="text" name="options[{{ option.id }}]" value="{{ product.price }}" {{ option.attributes|raw }} /></div>\n{% else %}\n<h4 class="donation-status">{% if product and product.donation and product.donation.completed %}Donation Exceed Target Amount{% else %}Donation Exceed Target Date{% endif %}</h4>\n{% endif %}`);
  writeOpt('image', `<input name="options[{{ option.id }}]" class="option-image" {% if option.value %}data-default="{{ option.value }}"{% endif %} {{ option.attributes|raw }} {{ option.condition_attributes|raw }} type="file" />`);
  writeOpt('multiple-options', `{% if option and option.details %}\n<div class="option option-multiple">{% for key,detail in option.details %}<label><input {{ option.disabled?'disabled':'' }} {{ detail.is_selected?'checked':'' }} name="options[{{ option.id }}]" data-option="{{ option.id }}" value="{{ detail.id }}" type="checkbox" {{ option.attributes|raw }} {{ option.condition_attributes|raw }} /> <span>{{ detail.full_name | default(detail.name) }}</span></label>{% endfor %}</div>{% endif %}`);
  writeOpt('number', `<input placeholder="{{ option.placeholder }}" name="options[{{ option.id }}]" value="{{ option.value }}" type="text" {{ option.attributes|raw }} {{ option.condition_attributes|raw }} />`);
  writeOpt('single-option', `<select name="options[{{ option.id }}]" data-option="{{ option.id }}" {{ option.attributes|raw }} {{ option.condition_attributes|raw }}><option placeholder value="">{{ option.placeholder }}</option>{% for key,detail in option.details %}<option {{ detail.is_selected ? 'selected' : '' }} value="{{ detail.id }}">{{ detail.full_name | default(detail.name) }}</option>{% endfor %}</select>`);
  writeOpt('splitter', `<div class="splitter"></div>`);
  writeOpt('text', `<input placeholder="{{ option.placeholder }}" name="options[{{ option.id }}]" {{ option.attributes|raw }} {{ option.condition_attributes|raw }} value="{{ option.value }}" type="text"/>`);
  writeOpt('textarea', `<textarea placeholder="{{ option.placeholder }}" name="options[{{ option.id }}]" {{ option.attributes|raw }} {{ option.condition_attributes|raw }}>{{ option.value }}</textarea>`);
  writeOpt('thumbnail', `{% if option and option.details %}{% for detail_index, detail in option.details %}<input {{ detail.is_selected?'checked':'' }} required type="radio" id="option_{{ detail.id }}-{{ option.id }}" {{ detail.is_out?'disabled':'' }} data-itemid="{{ detail.id }}" name="options[{{ option.id }}]" data-img-id="{{ detail.option_value }}" value="{{ detail.id }}" {{ option.condition_attributes|raw }} /> <img data-src="{{ detail.image }}" src="{{ asset('images/s-empty.png') }}" class="object-cover h-full w-full lazy-load" title="{{ detail.name }}" alt="{{ detail.name }}" /> <p>{{ detail.name }}</p> {% if detail.is_out %}<small>Out of stock</small>{% endif %}{% endfor %}{% endif %}`);
  writeOpt('time', `<input placeholder="{{ option.placeholder }}" name="options[{{ option.id }}]" {{ option.attributes|raw }} {{ option.condition_attributes|raw }} readonly="readonly" value="{{ option.value }}" type="text"/>`);
