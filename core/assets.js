...
function main() {
  ...
  writeIfMissing(
    ...
    [
      ...
      "  if (!id) {",
      "    const tmp = await api._json(api.baseURL.replace(/\\\/$/,'') + '/cart', { method: 'POST', headers: api.headers });",
      "    id = (tmp && tmp.data && tmp.data.id) || tmp.id || '';",
      ...
      "    const total = (res && res.data && res.data.total && res.data.total.amount) || 0;",
      "    const cont = document.getElementById('cart-items');",
      "    if (cont){",
      "      cont.innerHTML = items.map(function(it){",
      "        var img = (it && it.image) || '';",
      "        var name = (it && it.name) || '';",
      "        var amount = (it && it.price && it.price.amount) || '';",
      "        var currency = (it && it.price && it.price.currency) || '';",
      "        return '<div class=\\\"cart-item\\\"><img src=\\\"'+img+'\\\" alt=\\\"'+name+'\\\"><span>'+name+'</span><span>'+amount+' '+currency+'</span></div>';",
      "      }).join('');",
      "    }",
      ...
      "  t.textContent = msg; setTimeout(function(){ if (t && t.parentNode) t.parentNode.removeChild(t); }, 2500);",
      "}",
  ...
  writeIfMissing(
    ...
    [
      ...
      "  var id = (p && p.id) || '';",
      "  var name = (p && p.name) || '';",
      "  return '<div class=\\\"product-card\\\" data-product-id=\\\"'+id+'\\\"><h3>'+name+'</h3><button class=\\\"quick-add-btn\\\" data-product-id=\\\"'+id+'\\\">Add to Cart</button></div>';",
      "}",
      "async function enhanceGrids(){",
      "  document.querySelectorAll('.product-grid[data-api-endpoint]').forEach(async function(grid){",
      "    try{",
      ...
      "  let t;",
      "  input.addEventListener('input', function(){ clearTimeout(t); t = setTimeout(async function(){",
      ...
      "  }, 300); });",
      "}",
      "window.addEventListener('DOMContentLoaded', function(){ enhanceGrids(); enhanceLiveSearch(); });"
    ].join('\n')
  );
  writeIfMissing(
