...
class SallaAdapter {
  ...
  async generateFromCanonical(canonicalPath) {
    ...
    await this._validateStructure(themeOutputDir);
  ...
  async _validateStructure(dir){ const must=[path.join(dir,'templates','index.twig'), path.join(dir,'theme.json')]; for (const p of must){ if (!(await fs.pathExists(p))){ console.warn(`⚠️  Missing expected file: ${p}`); } } }
