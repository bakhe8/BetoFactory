...
- Priorities:
  - Asset normalization (rewrite relative refs in CSS/HTML to normalized build paths)
  - Expand QA summary (images/styles/scripts counts, normalized assets)
  - Add `factory:build` wrapper (parse → validate → build → zip) under `src/cli/factory-build.cjs`
