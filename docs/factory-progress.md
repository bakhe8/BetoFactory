...
- Asset Normalization Phase 2 — HTML and CSS asset references are now rewritten to canonical-relative paths; QA summary expanded with counts and invalid refs; `assets-manifest.json` emitted.
...
- Asset Management — Deep copy in place; further normalization DONE. Additional edge-case handling (srcset, inline style attributes) remains optional.
...
- Phase 7 – Canonical Upgrade: multi-file support + recursive asset parser + path rewriting. ✅ Done
- Phase 8 – Adapter Refactor: modular adapter registry (Salla default; multi‑platform via SMART_PLATFORMS). ✅ Done
...
- Parsing & Canonical Conversion — 88%
- Adapter & Theme Generation — 80%
- Asset & File Management — 85%
- Watcher & Logging — 70%
...
- Priorities:
  - Watcher/queue/backoff integration in smart watcher
  - Edge-case normalization (srcset, inline style="background:url(...)" in HTML)
  - Expand tests around normalization and QA summary
...
- Phase 9 – Factory Dashboard: local web UI for upload/build/QA with progress and downloads. ✅ Done
- Phase 10 – QA Automation: visual diffs & auto schema checks post-build. ✅ Done
